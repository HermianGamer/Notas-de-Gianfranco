...
As you would expect, `print_names_once` will take half the time to run as `print_names_twice`. And in the real world of software engineering, doubling the speed is _awesome_. The funny thing about Big O analysis is that **we don't care**. We're academics™.
...
Constants affect _actual_ runtime, but in Big O analysis we drop them because they don't affect how the runtime _scales_.
