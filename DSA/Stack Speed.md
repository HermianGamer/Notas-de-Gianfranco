...
And you'd be right! A stack _is_ a list with fewer features, but that's the point. By restricting the ways we can interact with the data, we guarantee that certain operations are _blazingly fast_. Here are all the operations a typical stack supports, along with their Big O time complexity:
