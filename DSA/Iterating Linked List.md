...
We need to change which node is the next to be _yielded_, but the `set_next`method of the `Node` class changes which is the next to be _pointed_ to – don't use it!

By [overriding](https://en.wikipedia.org/wiki/Method_overriding) the `__iter__` method, Python will allow us to use a `for` loop to iterate over the linked list:
