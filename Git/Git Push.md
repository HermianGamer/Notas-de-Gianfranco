The `git push` command pushes (sends) local changes to any "remote" - in our case, [[GitHub Repository]]. For example, to push our local `main` branch's commits to the remote `origin`'s `main` branch we would run:

```bash
git push origin main
```

_You need to be authenticated with the remote to push changes.
